...
@section('content')
    <div class="row">
                    ...
                    @if (count($Categories) > 0)
						@foreach($Categories as $Category)
						 <tr id="categories-list">
						  <td nowrap>
								<button type="button" id="view_kpas" class="btn btn-primary  btn-xs" onclick="postData({{$Category->id}}, 'kpas');">KPAs</button>
								<button type="button" id="edit_category" class="btn btn-primary  btn-xs" data-toggle="modal" data-target="#edit-category-modal" data-id="{{ $Category->id }}" data-name="{{ $Category->name }}" data-weight="{{ $Category->weight }}"><i class="fa fa-pencil-square-o"></i> Edit</button>
                          </td>
						  <td>{{ $Category->name }} </td>
						  <td>{{ $Category->weight }} </td>
						  <td>
                              <button type="button" id="" class="btn {{ (!empty($Category->status) && $Category->status == 1) ? "btn-danger" : "btn-success" }} btn-xs" onclick="postData({{$Category->id}}, 'actdeac');"><i class="fa {{ (!empty($Category->status) && $Category->status == 1) ? "fa-times" : "fa-check" }}"></i> {{(!empty($Category->status) && $Category->status == 1) ? "De-Activate" : "Activate"}}</button>
                          </td>
						</tr>
						@endforeach
                    @else
        ...
        @include('appraisals.partials.add_new_category')
        @include('appraisals.partials.edit_category')
    </div>
@endsection
...
@section('page_script')
    <script>
		...
		{
			if (data == 'kpas')
				location.href = "/appraisal/kpa/" + id;
			else if (data == 'edit')
				location.href = "/appraisal/cat_edit/" + id;
			else if (data == 'actdeac')
				location.href = "/appraisal/cat_active/" + id;
		}
        $(function () {
            var categiryID;
            //Tooltip
            ...
            $('#edit-category-modal').on('show.bs.modal', function (e) {
                ...
                categiryID = btnEdit.data('id');
                var Name = btnEdit.data('name');
                var WeightI = btnEdit.data('weight');
                var modal = $(this);
                modal.find('#name').val(Name);
                modal.find('#weight').val(WeightI);
            ...
            function postModuleForm(formMethod, postUrl, formName) {
                ...
                $.ajax({
                    ...
                    success: function(success) {
                        ...
                        newModuleList += '<li id="active-category" class="list-group-item active"><b>' + success['new_template'] + '</b> <font class="pull-right">' + ' '+ ';</font></li>';
            ...
            $('#add-category').on('click', function() {
                postModuleForm('POST', '/appraisal/category', 'add-category-form');
            });

            $('#update-category').on('click', function() {
                postModuleForm('PATCH', '/appraisal/cat_edit/' + categiryID, 'edit-category-form');
            });
        });
    </script>
@endsection