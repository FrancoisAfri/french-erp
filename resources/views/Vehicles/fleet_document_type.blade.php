        ...
        <div class="col-md-12">
            <div class="box box-warning">
                <div class="box-header with-border">
                    <h3 class="box-title"> Fleet Document Type</h3>
                    ...
                    <div class="box-body">
                        <table class="table table-bordered">
                            ...
                            @if (count($fleetdocumentType) > 0)
                              @foreach ($fleetdocumentType as $document)
                               <tr id="categories-list">
                               <td nowrap>
                                        <button vehice="button" id="edit_compan" class="btn btn-warning  btn-xs" data-toggle="modal" data-target="#edit-package-modal" data-id="{{ $document->id }}" data-name="{{ $document->name }}" data-description="{{$document->description}}" ><i class="fa fa-pencil-square-o"></i> Edit</button>
                                  </td>
                                 ...
                                 <td><button type="button" class="btn btn-danger btn-xs" data-toggle="modal" data-target="#delete-documents-warning-modal" data-id="{{ $document->id }}"><i class="fa fa-trash"></i> Delete</button></td>
                                </tr>
                                   @endforeach
                               @else
                               <tr id="categories-list">
                        ...
                        <div class="alert alert-danger alert-dismissable">
                            ...
                            No Fleet Document Type to display, please start by adding a new Fleet Document Type..
                        </div>
                        </td>
                        </tr>
                           @endif
                    ...
                    <div class="box-footer">
                     ...
                     <button type="button" id="cat_module" class="btn btn-warning pull-right" data-toggle="modal" data-target="#add-document-modal">Add new Fleet Document Type </button>
                    </div>
             </div>
        </div>
   <!-- Include add new prime rate modal -->
        @include('Vehicles.partials.add_document_modal')
...
<script>
       function postData(id , data ){   
            if(data == 'actdeac') location.href = "/vehice/document_act/" + id;
        ...
        $(function () {
            ...
            $('#add-document-type').on('click', function() {
                //console.log('strUrl');
                var strUrl = '/vehice/add_document';
                ...
                var successMsgTitle = 'Fleet Document Type Added!';
            ...
            $('#edit_document').on('click', function () {
                var strUrl = '/vehice/edit_document/' + fleetID;
         ...
         modalAjaxSubmit(strUrl, objData, modalID, submitBtnID, redirectUrl, successMsgTitle, successMsg, Method);
            ...
            var documtsID;
                    $('#delete-documents-warning-modal').on('show.bs.modal', function (e) {
                        var btnEdit = $(e.relatedTarget);
                        documtsID = btnEdit.data('id');
                        var modal = $(this);
                    });

                    $('#delete_documents').on('click', function () {
                        var strUrl = '/vehice/document/'+ documtsID;
