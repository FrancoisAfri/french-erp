...
@section('content')
        ...
        <div class="col-md-12">
            <div class="box box-warning">
                <div class="box-header with-border">
                    <h3 class="box-title"> Incidents Type </h3>
                ...
                <!-- <form class="form-horizontal" method="POST" action="/hr/document"> -->
                    ...
                    <div class="box-body">
                        <table class="table table-bordered">
                            ...
                            @if (count($incidentType) > 0)
                              @foreach ($incidentType as $incident)
                               ...
                               <td nowrap>
                                        <button vehice="button" id="edit_compan" class="btn btn-warning  btn-xs" data-toggle="modal" data-target="#edit-package-modal" data-id="{{ $incident->id }}" data-name="{{ $incident->name }}" data-description="{{$incident->description}}" ><i class="fa fa-pencil-square-o"></i> Edit</button>
                                  </td>
                                     <td>{{ (!empty( $incident->name)) ?  $incident->name : ''}} </td>
                                     <td>{{ (!empty( $incident->description)) ?  $incident->description : ''}} </td>
                                  <td>
                                    <!--   leave here  -->
                                    <button vehice="button" id="view_ribbons" class="btn {{ (!empty($incident->status) && $incident->status == 1) ? " btn-danger " : "btn-success " }}
                        ...
                        <div class="alert alert-danger alert-dismissable">
                            ...
                            No Incident Type to display, please start by adding a new Fleet Incident Type..
                        </div>
                        </td>
                        </tr>
                           @endif
   ...
   <!-- Include add new prime rate modal -->
        @include('Vehicles.partials.add_incident_modal')
        @include('Vehicles.partials.edit_incident_modal')
...
<script>
       function postData(id , data ){   
            if(data == 'actdeac') location.href = "/vehice/incident_act/" + id;
        ...
        $(function () {
            ...
            $('#add-incident-type').on('click', function() {
                //console.log('strUrl');
                var strUrl = '/vehice/incident_type';
                var modalID = 'add-incident-modal';
                var objData = {
            ...
            $('#edit_incident').on('click', function () {
                var strUrl = '/vehice/edit_incident/' + fleetID;
